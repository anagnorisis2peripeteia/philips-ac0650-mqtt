...
async function runSetup(configPath) {
  ...
  try {
    ...
    console.log();
    console.log("2. Log in with your Philips account (the Air+ app account)");
    console.log("3. After login, the browser will try to open a com.philips.air:// URL");
    console.log("   and show a 'site can't be reached' error — that's expected.");
    console.log();
    console.log("   How to get the redirect URL:");
    console.log("   • Chrome/Edge: open DevTools (F12) → Network tab → look for");
    console.log("     a request that redirects to 'loginredirect' → copy the");
    console.log("     Location header value (starts with com.philips.air://)");
    console.log("   • Firefox: the address bar briefly shows the com.philips.air:// URL");
    console.log("     before the error — copy it from there");
    console.log();
