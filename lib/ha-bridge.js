...
class HABridge {
  constructor(options = {}) {
    ...
    this.mqttUser = options.mqttUser || null;
    this.mqttPass = options.mqttPass || null;
  ...
  _mqttPublish(topic, payload, retain = false) {
    ...
    const packet = buildPublish(topic, payload);
    if (retain) {
      packet[0] |= 0x01; // Set retain bit
    }
    this.socket.write(packet);
  }
  ...
  _mqttSubscribe(topic) {
    if (!this.socket || !this.connected) return;
    const packet = buildSubscribe(topic, this.packetId++);
    this.socket.write(packet);
  }
